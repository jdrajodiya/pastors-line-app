...
const AllCountryContacts = ({ closeModal, handleModalB }: Props) => {
	...
	useEffect(() => {
		if (pagination?.page) {
			dispatch(getAllContactsData({ ...pagination, companyId: PARAMS.companyId }));
		}
		...
		<>
			...
			>
				<Row className="pb-3">
					...
					<Col xs={6} md={3}>
						<button className="btn secondary w-100" onClick={handleModalB}>
							US Contacts
						</button>
					</Col>
				</Row>
...
export default AllCountryContacts;
